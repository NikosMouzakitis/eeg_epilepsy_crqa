...
\end{comment}
	...
	\newpage
			...
			CRQA is performed to quantify the dynamical 
			interactions among the pairs of EEG channels 
			for each segment. The \texttt{pyrqa} library is 
			used to compute CRQA metrics, 
			which analyze the recurrence properties of two time series 
			in a shared phase space. 
			The process involves embedding the 
			time series, 
			computing the cross recurrence matrix, 
			and extracting the quantitative features.
			...
			The algorithm is summarized in Algorithm \ref{alg:crqa_computation}.
					...
					\begin{algorithmic}[1]
					\State \textbf{Input}: EEG segments \( \{X_{c,i}\} \) for channels \( c \in C \), segments \( i = 1, \dots, N_s \), where \( N_s = 5 \), number of electrodes \( N_e = 3 \)
		...
		For each segment and each pair of selected channels, the following steps were executed:
			...
			\begin{enumerate}
			    ...
			    \item \textbf{CRQA Computation}: Using the \texttt{PyRQA} library, a cross-recurrence plot is constructed 
					...
					The CRQA computation yields 16 quantitative features: 
					recurrence rate (RR), determinism (DET), average diagonal line length (\(L_{avg}\)), longest diagonal line (\(L_{max}\)), divergence (DIV), entropy of diagonal lines (\(H_{diag}\)), laminarity (LAM), trapping time (TT), longest vertical line (\(V_{max}\)), average white vertical line length (\(W_{avg}\)), longest white vertical line (\(W_{max}\)), inverse of the longest white vertical line (\(W_{max}^{-1}\)), entropy of vertical lines (\(H_{vert}\)), entropy of white vertical lines (\(H_{wvert}\)), ratio of determinism to recurrence rate (DET/RR), and ratio of laminarity to determinism (LAM/DET).
