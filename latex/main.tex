...
\begin{document}
	...
	These seizures are result of abnormal neuronal brain activity which subsequently lead into 
	disturbances in behavior, sensation, movement, or consciousness of the affected subject.
	...
	Epilepsy's causes are diverse and related to many factors. 
	For some patients, experienced seizures generated from identifiable structural, 
	genetic, infectious, metabolic, or immune-related abnormalities, while in others the trigger of seizures remains unknown. 
	Depending on the brain regions involved, a seizure may have several classifications. 
	It may be classified as focal (originating in a specific area) or a generalized one (involving both hemispheres). 
	Clinically, seizure manifestations present high variability, from brief lapses in awareness to 
	convulsive episodes, and their unpredictable occurrence has severe impact in patient’s quality of life.

	For the diagnosis and monitoring of epilepsy, electroencephalography (EEG) is widely utilized, as is offers a non invasive 
	...
	nonlinear dynamics, recurrence analysis, and deep neural networks offer solutions for automated extraction of complex spatial and temporal 
	features from EEG recordings. These approaches aim to support clinicians by providing objective, 
	data-driven tools for seizure detection, prediction, and classification, ultimately contributing to 
	better patient care and treatment personalization.
...
%				\end{itemize}
				...
				\subsection{Challenges: Class Imbalance in RQA-EEG Data}
				...
				yielding a $\sim$58:1 ratio. This skew mirrors clinical datasets like
				CHB-MIT but exacerbates classification challenges.

				Standard classifiers, such as Random Forest, achieve misleadingly high accuracy ($\sim$99\%) 
				by over-predicting the majority class, resulting in poor recall for epileptic events.
				In RQA-based models, this bias can obscure discriminative patterns, 
				such as the elevated determinism (DET) or laminarity (LAM) in epileptic signals \cite{acharya2012rqa}. 
				False negatives pose ethical risks, delaying interventions, while naive oversampling 
				(e.g., duplication) risks overfitting to artifacts in minority RQA features like entropy lines (L\_entr).

				In order to mitigate this, a data-level resampling strategy can be employed, focusing on 
