...
\end{comment}
	...
	\newpage
		...
		\section*{The Challenge of Phase Space Reconstruction}

			The analysis of nonlinear dynamical systems from experimental time series, such as electroencephalographic (EEG) recordings, requires reconstruction of the underlying phase space from scalar measurements. According to Takens' embedding theorem \cite{takens1981}, a time series $x(t)$ can be embedded in an $m$-dimensional space using time-delay coordinates:
			...
			\subsection*{The Problem of False Neighbors}

			When the embedding dimension $m$ is too small, the phase space becomes \emph{projected} rather than properly \emph{embedded}. This projection creates artificial neighborhoods where points appear close due to geometrical constraints rather than dynamical similarity. These are termed \emph{false nearest neighbors} \cite{kennel1992}.
