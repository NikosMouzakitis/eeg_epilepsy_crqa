...
\end{comment}
	...
	\end{figure}
		...
		In order to estimate and standardize a value for radius fraction R, an exploration
		of its effect on CRPs is performed. By keeping constant $\tau = 1$ and \textbf{\textit{m}} = 3
		cross recurrence plots are calculated and recurrence rate and determinism features are recorded.
		The following table presents the comparisson of the recurrence rate and determinism
		for different epileptic and normal windows exploring different values for the
		percentage threshold(\textit{radius\_fraction} R. Radius fractions is set on values
		{ 0.1, 0.15, 0.20 and 0.30} on this experiment.
		After calculating for the randomly choosen window of the 
		recording the 22x22 CRPs among its
		channels, the channel-wise mean RR and mean DET is calculated.
