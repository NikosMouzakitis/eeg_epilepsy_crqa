...
\begin{abstract}
	...
	hold the ability to capture informative seizure related 
...
%	and developing reliable methods for automatic EEG analysis 
	...
	The main goal of this thesis is to estimate whether cross-recurrence dynamics 
	extracted from multi-channel scalp EEG are capable to
	provide efficient subject-independent 
	discrimination between epileptic and non-epileptic 
	brain activity.

	Based on this, the central research question 
	that has been studied in this thesis is:

	\begin{quote}
	\textit{Can globally aggregated multichannel Cross-Recurrence Quantification Analysis (CRQA) features, extracted from scalp EEG recordings, 
		support reliable and statistically significant seizure detection performance 
		under strict Leave-One-Subject-Out (LOSO) evaluation?}
	\end{quote}

	In order to answer this question, a complete nonlinear dynamical 
	pipeline is implemented, 
	including preprocessing, phase-space reconstruction, CRQA feature extraction for  all the EEG channel pairs, 
	feature aggregation in a global synchrony descriptor, and machine learning classification. 
	Performance is evaluated under multiple validation schemes, by emphasizing on 
	subject-independent evaluation and
	statistical comparison across classifiers.
...
\midrule
1  & Frolov et al. (2020) & EEG & RQA, CRQA & Functional networks \\
...
15 & Fan and Chou (2019) & EEG & RQA, RN & Epilepsy \\
...
23 & Shalbaf et al. (2014) & EEG & CRQA (OPL) & Anesthesia monitoring \\
24 & Mihajlovi\'c (2019) & EEG & RQA & Cognitive tasks \\
25 & Gao et al. (2020) & EEG & RQA, Approx. Entropy + CNN & Epilepsy \\
\bottomrule
\end{tabularx}
...
%			recordings, and subjects prior to feature extraction.
			...
			In the CHM-MIT dataset, \textbf{136 recordings} in total from all the subjects, were identified 
			\textbf{containing} the desired 22 EEG signals and presenting \textbf{at least one epileptic seizure}.
	...
	\newpage
		...
		%\section{Phase space reconstruction}
			...
			based on the scalar measurements of each channel. This is a vital step,
			because recurrences are computed using the system's phase space.
		...
		\subsection{Calculation of time delay \( \tau \)}
			...
			\begin{figure}[H]
				    \centering
				    \includegraphics[width=0.6\linewidth]{ami.png}
				    \caption{Calculation of $\tau$ using AMI. The first minimum of the AMI function 
				    (green dashed line) is chosen as the optimal $\tau$.}
				    \label{fig:ami_plot}
			\end{figure}
	...
	\newpage
				...
				\section{RQA Metrics}
					...
					The presence of diagonal lines in a RP, means that a part of trajectory
					of the \texttt{solid} curve, evolves in time, within the thresholded distance 
					with the trajectory of the \texttt{dashed} line, as is visualized in Figure~\ref{fig:recurrencePS}.

					\begin{figure}[h!]
					    \centering
					    \includegraphics[width=0.5\linewidth]{recurrence_phase_space.png}
						\caption{Diagonal lines on RP; a close view on the phase space trajectory.}
					    \label{fig:recurrencePS}
					\end{figure}
				...
				\section{Cross-Recurrence Quantification Analysis (CRQA)}
					...
					In Figure \ref{fig:crpepileptic} an CRP of an epileptic window is presented.
					As can be observed, the presence of long and recurrent diagonal line segments 
					suggest extended periods during which both channels follow 
					similar trajectories in the phase space, reflecting coordinated dynamics. 
					The curvature and 
					fragmentation of these diagonals suggest that this synchronization 
					is nonstationary, but has evolving phase relationships over time. 
	...
	\newpage
				...
				In Listing {\ref{lst:segment_example}, a code snippet is presented which corresponds on the processing of 
				the 6th recording of patient 15, with summary information as in Figure~\ref{fig:patient_summary}, where the definition of the labels of a specific 
				...
				\begin{figure}[h!]
				    \centering
				    \includegraphics[width=0.7\linewidth]{patient_summary.png} %
					\caption{Database's annotated summary of recording.}
...
\end{lstlisting}
				...
				In order to define a recurrence, as seen in Equation 6.2, a distance threshold $ \varepsilon $ is required.
				This threshold is calculated as
				\[
				\varepsilon = R \cdot \operatorname{mean}\bigl(\mathrm{PhaseSpaceDiameter}(\text{electrode}_1),\,
				...
				\begin{table}[h!]
				\centering
				\caption{Comparison of RR and DET values for different radius fraction (R) values}
				...
				In all subsequent analysis the radius fraction value, was set to 0.15, in order to 
				achieve clear separation by preserving the contrast between recurrent and non-recurrent regions
				in the CRPs, and for avoiding the DET saturation effect.
				...
				of the whole EEG montage (a global synchrony descriptor). Under the hypothesis of seizure-related 
				...
				%\section{Classification - Experimental}
					...
					reduces variance when compared to LOSOCV by 
...
Support Vector Machines (SVMs) are supervised learning models used widely for classification and regression tasks\cite{svmCite}. 
The main concept of the SVM algorithm is the 
construction of a hyperplane in a higher dimensional space 
capable to separate different classes. 
The ultimate goal is to define the hyperplane 
that maximizes the margin between classes, 
that will lead in improved generalization performance.
...
the hard-margin(classes fully separable) SVM solves the following optimization problem:
...
slack variables $\xi_i \ge 0$ are utilized, giving the soft-margin SVM formulation:
...
where $\xi_i$ are the slack variables which allow misclassifications 
and $C>0$ is used controls the actual trade-off between the margin maximization 
and training error minimization.

As presented in Figure~\ref{svmmargin}\cite{svm_image}, SVM searches for the hyperplane 
that will maximize the margin among the two classes. 
From all the candidate separating hyperplanes, 
the one with the largest margin is selected, 
...
To evaluate the SVM classifier, an exploration of its 
hyperparameters was conducted in order to examine 
their influence on classification performance. 
SVMs include configurable options that control the geometry 
of the decision boundary, the complexity of the model and the optimization process.

\paragraph{Kernel Functions}

The kernel function defines the transformation that maps 
input data into a higher dimensional space where a 
linear separating hyperplane may exist. 
...
\]
		This kernel incorporates polynomial 
		combinations of the input features up to a specified degree, 
		enabling the model to capture non-linear feature interactions. 
		Using higher degrees increase the model’s capacity 
		but also raise the risk of overfitting. 
		
		Parameter $\gamma$ controls the contribution of the inner product, 
		while $\texttt{coef0}$ adjusts the influence of higher-order terms.
...
\item \textbf{RBF Kernel} (\texttt{rbf}):  
	...
	\]
	where $\gamma > 0$ determines the width of the Gaussian kernel and 
		controls how far the influence of a 
		single training sample reaches.

\end{itemize}

\paragraph{Other Optimization Parameters}

In this paragraph we describe other optimization parameters utilized in the Python code while executing the experiments.
...
To address class imbalance, the SVM was configured with \texttt{class\_weight="balanced"}, which scales class penalties inversely proportional to class frequency, ensuring that minority classes contribute equally to the loss function.
					...
					In the following Tables~\ref{tab:svm_comparison}, \ref{tab:svm_loso} and \ref{tab:svm_per_patient} the classification performance of SVM
					is reported.
				...
				\subsection{Effect of subject-level window imbalance}

			An osbervation on the window-based EEG classification performed, is the 
			not equal number of extracted windows per subject (Figure~\ref{fig:normal-epileptic-patient}).
			In the present dataset, this resulted in variability in the number of windows per patient, 
			with certain subjects (p6) contributing much more samples than others. 
			Although LOSOCV evaluation protocol was employed to 
			ensure subject-independent testing, 
			such window-level imbalance may still influence the training process, 
			as classifiers are optimized over individual windows and not on subjects.

			For assessing the impact of this effect, an additional analysis was conducted by 
			repeating the LOSO evaluation with excluding the most heavily represented subject (p6), 
			who also presented lower per-patient classification performance comparing to the other patients. 
			When this subject was removed, the mean classification accuracy increased 
			from $0.899 \pm 0.068$ to $0.905 \pm 0.062$, while the mean F1-score 
			improved from $0.885 \pm 0.093$ to $0.896 \pm 0.076$. 
			In the same manner UAR remained stable, 
			with slight increase from $0.904 \pm 0.059$ to $0.906 \pm 0.061$. 
			Sensitivity and specificity showed only negligible changes.

			These results indicate that, despite the imbalance in window counts across subjects, 
			the overall classification performance is not dominated by any single patient. 
			The limited performance gain observed after exclusion suggests 
			that the lower per-patient scores primarily reflect subject-specific signal characteristics and 
			not a systematic bias in the learning process. 
				...
				%	designed to reduce variance and improve generalization compared to a single decision tree. 
					...
					By using this dual randomization, this method decorrelates the trees and significantly improves 
				...
				%	For each hyperparameter configuration, the classifier was evaluated using a Leave-One-Subject-Out (LOSO) protocol to ensure strict subject-independent validation. Performance was quantified using accuracy, sensitivity (recall), specificity, and F1-score. Mean and standard deviation values across all subjects were computed to assess both predictive performance and inter-subject stability.
					...
					In the following Tables~\ref{tab:rf_comparison}, \ref{tab:rf_loso} and \ref{tab:rf_per_patient} the classification performance of Random Forest 
					is reported.
					...
					kNN can adapt naturally on complex data distributions and performs well when 
					representative samples are available in the feature space. 
					However, kNN is sensitive to feature scaling and noise, 
					making proper preprocessing essential.
					...
					\]
					This choice affects computational efficiency, particularly for high-dimensional feature spaces. In the following Tables~\ref{tab:knn_comparison}, \ref{tab:knn_loso} and \ref{tab:knn_per_patient} the classification performance of kNN is reported.
				...
				%\section{Classification Results and Comparative Analysis}
					...
					This section presents the classification performance obtained using three different machine learning models: SVM, RF and kNN. 
					All models were evaluated under a strict Leave-One-Subject-Out (LOSO) 
					cross-validation protocol to ensure subject-independent testing. 
					Performance was quantified using accuracy, sensitivity, specificity, F1-score, 
					and UAR and results are reported as mean and
					standard deviation across all subjects.


					\section{Comparison of Classifiers under Subject-Independent Evaluation}

The three classifiers, were evaluated using LOSOCV protocol. 
This setup ensures realistic estimation of generalization performance on 
unseen patients, which is critical in biomedical and 
physiological signal classification tasks.

Table~\ref{tab:loso_comparison} presents the mean and standard deviation of the main performance metrics 
across the 24 patients in LOSO evaluation.

\begin{table}[ht]
\centering
\small
\caption{LOSOCV classification performance comparison. Results are reported as mean $\pm$ standard deviation over 24 patients.}
\label{tab:loso_comparison}
\begin{tabular}{lccccc}
\hline
	\textbf{Classifier} & \textbf{Accuracy(\%)} & \textbf{Sensitivity(\%)} & \textbf{Specificity(\%)} & \textbf{F1-score(\%)} & \textbf{UAR(\%)} \\
\hline
SVM		& $89.9 \pm 6.8$ & $92.6 \pm 5.9$ & $88.2 \pm 10.9$ & $88.5 \pm 9.3$ & $90.4 \pm 5.9$ \\
Random Forest   & $85.8 \pm 8.3$ & $88.4 \pm 9.8$ & $83.8 \pm 15.7$ & $84.2 \pm 10.9$ & $86.1 \pm 7.9$ \\
kNN		& $84.2 \pm 7.9$ & $78.8 \pm 12.7$ & $89.5 \pm 9.3$ & $81.0 \pm 10.8$ & $84.1 \pm 7.2$ \\
\hline
\end{tabular}
\end{table}

In all the evaluated methods, SVM consistently achieves the best subject-independent performance. 
It shows the highest mean accuracy (+5.7\% over kNN and +4.1\% over RF), 
the highest sensitivity, the best balanced score (UAR), and importantly 
the lowest inter-patient variability 
(lowest standard deviation on accuracy). 

Random Forest shows advantage in 
sensitivity compared to kNN (+9.6\%), 
which may be preferable in applications where missing positive 
cases is particularly costly. But as we can observe, 
this comes with a payoff to lower specificity. 

kNN exhibits the weakest overall discriminative 
power in the LOSO setting, with the lowest sensitivity and 
highest variance in several metrics.

These results highlight that:
\begin{itemize}
    \item Strict subject-independent protocols (such as LOSO) should be regarded as the primary evaluation criterion.
    \item Patient-agnostic / random-split evaluation protocols overestimate performance by 6--8.5 percentage points across all models due to data leakage from the same subjects appearing in both training and test sets.
    \item For this particular classification task, SVM provides the most robust and reliable generalization to unseen patients.
\end{itemize}

			We can observe in Figure \ref{fig:svm_across_pat} the LOSOCV results that SVM classifier achieves for
			each patient examined.
					
					\begin{figure}[h]
					    \centering
					    \includegraphics[width=1.0\textwidth]{svm_across_pat.png}
					    \caption{SVM LOSOCV results.}
					    \label{fig:svm_across_pat}
					\end{figure}


					In Figure \ref{fig:loso_model}, we can observe the overall metrics 
					comparison between the different tested classifier's models. 
					\begin{figure}[h]
					    \centering
...
In order to determine whether the observed differences in classification performance were consistent across subjects and not due to random variation, a statistical comparison between classifiers was conducted. Since the LOSOCV protocol produces one performance score per patient for each model, paired statistical testing was applied across the 24 subjects.

The UAR was selected as the main evaluation metric, because of the robustness it has under class imbalance. 
Pairwise comparisons between classifiers were performed using 
the nonparametric Wilcoxon signed-rank test.
For controlling multiple comparisons across the three classifier pairs, 
Holm–Bonferroni correction was applied. 
Effect sizes were quantified using Cliff’s delta, 
and practical relevance was further assessed via 
the median paired performance difference.

The statistical results are summarized in Table~\ref{tab:stat_tests_loso}.

\begin{table}[H]
\centering
\caption{Pairwise statistical comparison of classifiers using per-patient LOSO UAR scores (n=24). Holm-corrected p-values are reported.}
\label{tab:stat_tests_loso}
\begin{tabular}{lcccc}
\hline
\textbf{Comparison} & \textbf{Median $\Delta$UAR} & \textbf{Cliff’s $\delta$} & \textbf{$p$ (Holm)} & \textbf{Interpretation} \\
\hline
SVM vs RF   & +0.032  & 0.83 & $2 \times 10^{-6}$ & Large effect \\
SVM vs kNN  & +0.072  & 0.83 & $1.9 \times 10^{-4}$ & Large effect \\
RF vs kNN   & +0.034  & 0.42 & 0.076 & Not significant \\
\hline
\end{tabular}
\end{table}

The results indicate that the SVM classifier significantly outperforms both Random Forest and kNN under strict subject-independent evaluation. The improvements are not only statistically significant but also practically meaningful, with large effect sizes (Cliff’s $\delta = 0.83$) in both comparisons. This demonstrates that SVM provides consistently superior generalization performance across unseen patients.

Although Random Forest shows a tendency to outperform kNN, the difference does not reach statistical significance after correction for multiple testing. Therefore, no definitive performance ordering can be established between these two models.

These findings reinforce the conclusion that SVM constitutes the most reliable classifier among those evaluated for recurrence-based seizure detection in a subject-independent setting.
...
Model stability across subjects was assessed through the standard deviation of classification metrics in the Leave-One-Subject-Out (LOSO) evaluation. Among the three classifiers, SVM exhibited the lowest variance on most performance metrics -- particularly accuracy ($\pm 0.068$), UAR ($\pm 0.059$), and sensitivity ($\pm 0.059$) -- indicating superior robustness to inter-subject variability. Random Forest showed moderate to high variance (accuracy $\pm 0.083$, specificity $\pm 0.157$), while kNN displayed the highest instability in sensitivity ($\pm 0.127$) and considerable variation overall, reflecting its strong dependence on local sample distributions.

The LOSO protocol represents a highly challenging validation setting, as the classifier must generalize to completely unseen subjects. The fact that all three models achieved consistent above-chance performance under this strict subject-independent protocol demonstrates that the extracted feature representations contain substantial discriminative, subject-independent information.
					...
					Recurrence Rate (RR) and Determinism (DET). 
					...
					Figure~\ref{fig:det_patient} show an example heatmaps for the DET and RR features, 
					...
					\section*{Recurrence Quantification Analysis and Patient Difficulty}

To gain deeper insight into the inter-subject variability 
observed in the LOSOCV results, 
RQA metrics are visualized using the scatter plot of recurrence rate (RR) against determinism (DET).
Plots reveal a separation between "good" and "bad" patients (Figures~\ref{fig:rqa-scatter1}, \ref{fig:rqa-scatter2}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.5\textwidth]{rdetscatter2.png}
\caption{RR vs. DET for a "good" performer.}
\label{fig:rqa-scatter1}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.5\textwidth]{rdetscatter1.png}
\caption{RR vs. DET for a "bad" performer.}
\label{fig:rqa-scatter2}
\end{figure}


Patients classified as "good" (e.g., p22) consistently present 
distinct separation between the two classes, with epileptic segments
having higher recurrence rate and higher determinism than the normal ones.
This pattern translates in more clear discrimination 
even under 
strict subject-independent validation.

In contrast, "bad" or difficult patients 
(e.g., p6) appear having both classes mixed sharing a large number of common values 
for both RR and DET. 
This non-separation leads into poorer generalization 
when the model has never seen data from that subject before.

A representative example is patient \textbf{p6}, who exhibits some of the lowest 
LOSOCV performances across all of the classifiers:
\begin{itemize}
    \item SVM: Accuracy 0.701, Sensitivity 0.947, F1 0.573, UAR 0.791
    \item Random Forest: Accuracy 0.699, Sensitivity 0.920, F1 0.563, UAR 0.780
    \item kNN: Accuracy 0.741, Sensitivity 0.787, F1 0.562, UAR 0.758
\end{itemize}

This behavior aligns with p6's visualization  on its scatter plot.
			...
			\section{Recording Session Variability}

			While subject-independent generalization was addressed using a Leave-One-Subject-Out (LOSO) 
			evaluation protocol, another source of variability in EEG analysis has its origins
			on the differences between recording sessions. 
			Even for the same subject, EEG signals can vary across sessions, because of changes in the placement
			of electrodes,
			impedance and quality of electrode contact, 
			reference configuration, hardware characteristics, 
			and subject’s physiological or behavioral state \cite{Huang2023,Apicella2024}. 
			These session-level differences can alter signal amplitude, 
			spectral characteristics, 
			noise profiles, and inter-channel relationships in a manner non-dependent on the 
			underlying brain dynamics.
			CRQA features provided the ability to quantify dynamical 
			coupling between channels with a global synchrony descriptor, but
			metrics may be sensitive also in session-dependent recording conditions. 
			Therefore, while the LOSO protocol, 
			ensures generalization across subjects, 
			it does not posses any control on the variability introduced by different recording sessions. 
			This remains an important consideration for real-world deployment in the application domain.
		...
		%\section{Future Work}
			...
			Although the present thesis evaluated the ability of how the 
