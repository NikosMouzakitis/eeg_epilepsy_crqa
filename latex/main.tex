...
\begin{document}
	...
	\section{Related Work}
		...
		\end{table}
			...
			%% PATENTS
			\subsection{RQA relevant patents for EEG seizure detection}
			...
			Notably, the patent by \cite{pat2018}, titled 
			...
			The inventors propose a hybrid model consisting of three key stages:
				...
				\begin{enumerate}
				    \item \textbf{Feature Extraction:} The method employs a multi-modal feature extraction strategy. Crucially, it explicitly includes \textit{Recurrence Quantification Analysis (RQA)} for extracting dynamic non-linear features from the EEG signal, alongside more traditional time-domain features (e.g., root mean square, zero-crossing rate) and frequency-domain features discovered via a genetic algorithm.
				    \item \textbf{Feature Optimization:} A Genetic Algorithm (GA) is used for feature selection. The algorithm uses binary encoding (a string of 1s and 0s) to represent chromosomes, where each bit corresponds to the selection (1) or rejection (0) of a specific feature from the large extracted pool. This optimizes the feature subset for maximum discriminative power.
				    \item \textbf{Classification:} The optimized feature set is fed into a Random Forest classifier for final prediction. The combination of GA for selection and Random Forest for classification is chosen for its ability to handle high-dimensional data and reduce overfitting.
				\end{enumerate}
