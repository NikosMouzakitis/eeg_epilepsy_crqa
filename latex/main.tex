...
\begin{document}
	...
	Epilepsy has diverse and various causes, since they are related to many factors. 
	...
	Clinically, seizures have high variability, 
	...
	\section{Related Work}
		...
		\end{table}
			...
			The application of RQA on the biomedical field 
			related to EEG data, 
			...
			the cutting-edge, industrially viable solutions 
...
%			implementing RQA in a resource-constrained environment.
	...
	%		Also the described device adjusts or turns off the stimulation based on the patient's sleep state in order to save battery life and reduce side effects.
		...
		All the recordings of the database were collected at the Children's Hospital Boston.
		The dataset contains multiple cases (patients), each with long-term scalp EEG signals recorded using the international 10–20 system.
		Table~\ref{tab:chbmit_demographics} summarizes the demographic information (case ID, gender, and age) for all subjects included in the database.
		The database is used in many studies for testing algorithms on epileptic seizure 
		...
		\noindent
		The database includes recordings from both male and female patients, where the ages are in the range of 1.5 to 22 years old.
		Most subjects are children and adolescents, reflecting the pediatric nature of the dataset.
		This demographic diversity provides a representative sample for studying epileptic activity across different developmental stages.
		...
		but in some files there are extra channels records, such as ECG and EMG references. 
		The sampling rate of the recordings is 256 Hz with 16-bit resolution. 
		...
		\section{Filtering}
			...
			Electroencephalogram signals in most cases 
			...
			Also hybrid approaches (f.e, wavelet-ICA) combine multiple techniques for 
