...
%	and developing reliable methods for automatic EEG analysis 
	...
	that has addressed in this thesis is:
...
As presented in Figure~\ref{svmmargin}, SVM searches for the hyperplane 
...
The three classifiers --- k-Nearest Neighbors (kNN), Random Forest (RF), and Support Vector Machine (SVM) with RBF kernel --- were evaluated using a strict Leave-One-Subject-Out (LOSO) cross-validation protocol. This setup ensures realistic estimation of generalization performance to completely unseen patients, which is critical in biomedical and physiological signal classification tasks.

Table~\ref{tab:loso_comparison} presents the mean and standard deviation of the main performance metrics across the 24 patients in LOSO evaluation.

\begin{table}[ht]
\centering
\small
\caption{Leave-One-Subject-Out (LOSO) classification performance comparison across the three classifiers. Results are reported as mean $\pm$ standard deviation over 24 patients.}
...
In order to determine whether the observed differences in classification performance were consistent across subjects and not due to random variation, a statistical comparison between classifiers was conducted. Since the Leave-One-Subject-Out (LOSO) protocol produces one performance score per patient for each model, paired statistical testing was applied across the 24 subjects.
...
This behavior aligns with p6's visualization  on its scatter plot.
			...
			evaluation protocol, another source of variability in EEG analysis arises 
			from differences between recording sessions. 
			Even for the same subject, EEG signals can vary across sessions due to changes in electrode placement, 
			impedance and contact quality, 
			reference configuration, hardware characteristics, 
			and the patient’s physiological or behavioral state \cite{Huang2023,Apicella2024}. 
			These session-level differences can alter signal amplitude, spectral characteristics, 
			noise profiles, and inter-channel relationships independently of the underlying neural dynamics.
			CRQA features provide the ability to quantify dynamical coupling between channels. 
			Such coupling metrics may be sensitive not only to seizure synchronization but 
			also in session-dependent recording conditions. 
			Therefore, while the LOSO protocol ensures generalization across subjects, 
			it does not control the variability introduced by separate recording sessions. 
