...
%	\includegraphics[width=6cm]{ELMEPA.png}}\\[1cm]
	...
	disturbances in the behavior, sensation, movement as well as in the consciousness
	...
	Epileptic seizures are not local phenomena 
	but involve the abnormal synchronization and 
	interaction between different "distributed" brain regions\cite{sync1}. 
	As a result, analyzing each EEG channel independently may miss
	important information related to the way neural populations interact 
	...
	Dynamical systems or time series are 
	considered stationary when their statistical 
	properties remain constant over time. 
	In more detail, in a stationary process the mean, variance, and autocorrelation 
	structure do not change as time evolves.
	In such systems, fluctuations occur around a stable equilibrium point and 
	the probabilistic behavior of the signal is time-invariant.  On the other hand, non-stationary system 
	is one with changing statistical properties change over time, due to transitions in the underlying dynamics.
	Non-stationarity applies also to 
	EEG, in which the brain activity continuously performs switches among different functional states.
	This nature of EEG signal, serves as  motivation, for analyzing it utilizing recurrence-based methods
	which do not assume stationarity and are able to detect dynamic transitions.	

	Summarizing, epilepsy represents a major public health challenge due to its prevalence, 
	...
	The full methodological pipeline, including feature extraction/aggregation, is presented in Section 7. 
	...
	\section{Related Work}
			...
			particularly when analyzing coupling across brain regions.
			...
			brain connectivity using multichannel EEG signals. In this work,
			...
			Beyond seizure diagnosis, recurrence analysis has also been utilized 
			in studies linking neural dynamics to cognitive function.
			In their study, Mo et al.\cite{mo} used sEEG 
			combined with RQA for the examination of the relationship of the DMN and empathy. 
			...
			Ngamga et al.\cite{ngamga2016} studied the performance achieved of RQA and Recurrence 
			Network (RN) measures in identifying 
			pre-seizure states from multi-day, multi-channel intracranial EEG (iEEG) 
			recordings of epilepsy patients. 
			Results pointed out the correlation among RQA measures 
			(determinism, laminarity, and mean recurrence time) in 
			detecting seizure precursors, while RN measures 
			(average shortest path length and network transitivity) provided 
			...
			This study supports RQA's sensitivity for nonlinear dynamics 
		...
		%	performance than structural measures, particularly for verbal episodic memory. 
			...
			(\textit{recurrence rate, laminarity inspired}
			(custom feature)\textit{, and average structure size}) complement 
			...
			Several works have demonstrated that recurrence-based and cross-recurrence measures 
		...
		\end{table}
			...
			%% PATENTS
			\subsection{RQA patents utilizing EEG modality}
			...
			that bypasses explicit construction of the full \textbf{N×N}
			...
			First, EEG features are extracted by a multi-modal strategy combining RQA measures 
		...
		\section{Filtering}
			...
			\subsection{Evaluation metrics for EEG denoising}
...
%			recordings, and subjects prior to feature extraction.
	...
	\newpage
		...
		\subsubsection{Calculation of time delay \( \tau \)}
			...
			\subsubsection{Estimating embedding dimension \( m \)}
...
%					suggests strong potential for subject-independent deployment.
					...
					\subsection*{Temporal Evolution of CRQA Features}
					...
					\begin{figure}[H]
					    \centering
					    \includegraphics[width=0.6\textwidth]{p1_18.png}
					    \includegraphics[width=0.6\textwidth]{p11_99.png}
					...
					Figure \ref{fig:temp_evol} illustrates a representative example, presenting the evolution of the CRQA metrics against time samples
					for two of the analyzed recordins. Several measures exhibit clear transitions 
					during epileptic periods. Most notable, determinism (DET), recurrence rate (RR), and laminarity (LAM) 
					tend to increase during seizure segments, indicating the shift toward more structured and 
					recurrent dynamics. 
					Simultaneously, measures like divergence tend to decrease on global minumum ranges.
					These observations support the hypothesis that epileptic seizures are associated with increased dynamical 
			...
			epileptic EEG segments were characterized by increased 
			...
			In contrast, normal EEG segments exhibited more irregular
			...
			\subsection{Constant embedding parameters}
			...
			\section{Future Work}
				...
				While constant parameters were chosen in this thesis to ensure stability and consistency, 
