...
%			implementing RQA in a resource-constrained environment.
	...
	\newpage
				...
				where each element is representing the average value of a specific 
				CRQA metric across 
				the entire EEG montage for this window.
				...
				\section{Classification - Experimental}
					...
					\textit{Accuracy} represents the proportion of correctly classified instances over the total number of instances:

					\begin{equation}
					\text{Accuracy} = \frac{TP + TN}{TP + TN + FP + FN}
					\end{equation}

					\textit{Sensitivity}, known also as recall or true positive rate, measures the proportion of actual positives correctly identified:

					\begin{equation}
					\text{Sensitivity} = \frac{TP}{TP + FN}
					\end{equation}

					\textit{Specificity}, also known as true negative rate, measures the proportion of actual negatives correctly identified:

					\begin{equation}
					\text{Specificity} = \frac{TN}{TN + FP}
					\end{equation}

					\textit{Precision} measures the proportion of predicted positives that are correctly identified:

					\begin{equation}
					\text{Precision} = \frac{TP}{TP + FP}
					\end{equation}

					The \textit{F1-score} is the harmonic mean of precision and recall, providing a single metric that balances both:
					...
					overall accuracy is provide to provide misleading estimation for the performance,
					because of the domination of majority class.
					To address this issue, the \emph{Unweighted Average Recall (UAR)} metric has also been employed.

					For a binary classification tasks, 
					UAR is defined as the arithmetic mean of sensitivity (recall of the positive class) and 
					specificity (recall of the negative class). 
					The Unweighted Average Recall is defined as
					\begin{equation}
					...
					\subsection{Classification Evaluation Methods}
					

					\subsection{Comparison of Evaluation Protocols}
					Three complementary classification evaluation protocols were employed: 
					(i) repeated random sample-level splitting (80\%–20\%), (ii) repeated subject-level 
					random splitting (19 training subjects, 5 testing subjects), and 
					(iii) Leave-One-Subject-Out (LOSO) cross-validation.

					The sample-level protocol provides an optimistic upper bound on classification performance, as samples from the same subject may appear in both training and test sets. In contrast, the subject-level protocols enforce strict subject independence, thereby assessing generalization to unseen subjects.

					The repeated subject-level random splitting approach reduces variance compared to LOSO by averaging performance over multiple randomly selected test subject subsets, while LOSO represents the most conservative evaluation strategy by testing on a single held-out subject in each fold.

					Taken together, these evaluation protocols offer a comprehensive characterization of classifier performance, ranging from idealized conditions to strict subject-independent generalization.

					\subsubsection{Leave-One-Subject-Out (LOSO) Cross-Validation}

					To obtain an unbiased estimation of the ability that a classifier has on generalizing among different individuals, 
					the experiments conducted, utilized the 
					Leave-One-Subject-Out (LOSO) cross-validation. 
					In LOSO, data from one subject (patient) 
					are held out as the test set, 
					while data from all remaining subjects are used for training. 
					The model is always evaluated on unpresented data to it.
					This process is repeated iteratively until every subject has 
					served as the test set exactly one time.

					Training and evaluating using different subjects for each case,
					ensures that the classifier does not learn specific patterns presented in already seen subjects 
					that would not generalize later on to unseen individuals. 
					As a consequence, LOSO method is a conservative and realistic estimate of 
					the performance in real life, where new patient data must be 
					classified without retraining the model.

					For each hyperparameter configuration explored, the models were trained and tested across 
					all LOSO folds. Performance metrics (accuracy, sensitivity, specificity, F1-score and UAR) are 
					computed for each held-out subject and then averaged in order to obtain the final estimate. 

					\subsubsection{Repeated Random Sample-Level Evaluation}

					In addition to leave-one-subject-out validation, a repeated random sub-sampling
					evaluation strategy was employed.
					At each repetition, the entire dataset was randomly partitioned into a training
					...
					Performance metrics, including accuracy, sensitivity, specificity, F1-score,
					and unweighted average recall (UAR), were computed for each repetition and
					subsequently aggregated by reporting the mean and standard deviation across
					all repetitions.

					This evaluation strategy provides an estimate of the expected performance under
					random data partitioning, while reducing the variance associated with a single
					train–test split.
					However, since samples from the same subject may appear in both training and
					test sets, this approach does not fully account for inter-subject variability
					and therefore represents an optimistic performance estimate.

					\subsubsection{Repeated Subject-Level Random Evaluation}

					To assess subject-independent generalization while mitigating the high variance
					of leave-one-subject-out validation, a repeated subject-level random evaluation
					scheme was adopted.
					At each repetition, 19 subjects were randomly selected for training and the
					remaining 5 subjects were reserved exclusively for testing.
					All samples belonging to a subject were assigned to the same subset, ensuring
					strict subject-level separation between training and test data.

					This process was repeated 500 times for each classifier configuration.
					For each repetition, the classifier was trained using data from the training
					subjects and evaluated on data from the held-out subjects.
					Performance metrics (accuracy, sensitivity, specificity, F1-score, and UAR)
					were computed and summarized using their mean and standard deviation across
					all repetitions.

					This evaluation strategy balances subject independence with statistical
					stability and provides a more robust estimate of generalization performance
					compared to a single subject-wise split.
